...
export class MarkdownParser {
  ...
  static _parseParagraphs(text) {
    ...
    const splitText = text.split(/(<\/?(?:h[1-6]|ul|ol|li|blockquote|div|pre|table|tr|td|th)[^>]*>)/g);
    ...
    for (let i = 0; i < splitText.length; i++) {
      ...
      if (part.match(/<(?:h[1-6]|ul|ol|li|blockquote|div|pre|table|tr|td|th)[^>]*>/)) {
        inBlock = true;
        result.push(part);
      } else if (part.match(/<\/(?:h[1-6]|ul|ol|li|blockquote|div|pre|table|tr|td|th)[^>]*>/)) {
        inBlock = false;
        result.push(part);
      } else if (!inBlock) {
  ...
  static _parseTable(tableText) {
    ...
    const alignments = [];
    const alignmentCells = lines[1].split('|').filter(cell => cell.trim() !== '');
